(function (module) {
  ...
  function applyImage() {
    ...
    if (_rawWidth === 0 && _rawHeight === 0)
      console.log("failed to read image dimensions");
    if (
      _rawWidth < RENDERED_IMAGE_WIDTH ||
      _rawHeight < RENDERED_IMAGE_HEIGHT
    ) {
      clearImage();
      _setSizeAlert(true);
      return;
    ...
    let landscape =
      _rawWidth / _rawHeight >= PREVIEW_IMAGE_WIDTH / PREVIEW_IMAGE_HEIGHT;

    const big = landscape ? _rawWidth : _rawHeight;
    const small = landscape ? _rawHeight : _rawWidth;
    aspectRatio = big / small;

    _resizeRatio = landscape
      ? _rawHeight / RENDERED_IMAGE_HEIGHT
      : _rawWidth / RENDERED_IMAGE_WIDTH;

    // TODO: change _canvasWidth to PREVIEW_IMAGE_WIDTH, etc
    const finalWidth = landscape
      ? (_canvasWidth * aspectRatio) / initialAspectRatio
      : _canvasWidth;
    const finalHeight = landscape
      ? _canvasHeight
      : _canvasHeight * aspectRatio * initialAspectRatio;
  ...
  function setSize(setWidth, setHeight) {
    const collectionOfElements =
      document.getElementsByClassName("variable-size");
  ...
  function drawOverlay(event) {
    ...
    if (!event) {
      ...
      if (event.touches && event.touches.length > 0) {
        _currentX =
          _previousX + (event.touches.item(0).pageX - _initialPointerX);
        _currentY =
          _previousY + (event.touches.item(0).pageY - _initialPointerY);
      }
    }

    // Keep overlay rect in the frame
    if (_currentX < cropHalf.width) _currentX = cropHalf.width;
    if (_currentX > _canvasWidth - cropHalf.width)
      _currentX = _canvasWidth - cropHalf.width;
    if (_currentY < cropHalf.height) _currentY = cropHalf.height;
    if (_currentY > _canvasHeight - cropHalf.height)
      _currentY = _canvasHeight - cropHalf.height;
    ...
    ctx.strokeRect(
      _currentX - cropHalf.width,
      _currentY - cropHalf.height,
      cropSize.width,
      cropSize.height
    );
    ...
    ctx.fillRect(
      0,
      0,
      landscape ? _currentX - cropHalf.width : _canvasWidth,
      landscape ? _canvasHeight : _currentY - cropHalf.height
    );
    ctx.fillRect(
      landscape ? _currentX + cropHalf.width : 0,
      landscape ? 0 : _currentY + cropHalf.height,
      canvas.width,
      canvas.height
    );
    ...
    _overlayOriginX =
      (_currentX - cropHalf.width) *
      (RENDERED_IMAGE_WIDTH / PREVIEW_IMAGE_WIDTH);
    _overlayOriginY =
      (_currentY - cropHalf.height) *
      (RENDERED_IMAGE_HEIGHT / PREVIEW_IMAGE_HEIGHT);
  }
  ...
  function drawCroppedCanvas() {
    const croppedCanvas = document.createElementNS(
  ...
  function getImageSize(arrayBuffer, imageType) {
    ...
    function subIFD(startByte, offsetToFirstIFD, i, isIntelAlign) {
      let IFDWidth = 0;
      let IFDHeight = 0;
      let subOffset = dataV.getUint16(
        startByte + 9 + offsetToFirstIFD + 2 + i * 12 + (isIntelAlign ? 8 : 10),
        isIntelAlign
      );
      ...
      for (let n = 0; n < numOfSubs; n++) {
        let entryItem = dataV.getUint16(
          fullSubOffset + 2 + n * 12,
          isIntelAlign
        );

        if (entryItem === EXIF_SUB_IFD_WIDTH) {
          IFDWidth = dataV.getUint32(
            fullSubOffset + 2 + n * 12 + 8,
            isIntelAlign
          );
      ...
      for (let x = 0; x < 40000; x++) {
        if (dataV.getUint8(x) === MARKER_PREFIX) {
          ...
          if (dataV.getUint8(byte) === EXIF_APP1_MARKER) {
            ...
            if (
              dataV.getUint8(byte + 9) === INTEL_BYTE &&
              dataV.getUint8(byte + 10) === INTEL_BYTE
            )
              isIntelAlign = true;

            // Offset to the first IFD
            const offsetToFirstIFD = dataV.getUint32(byte + 13, isIntelAlign);
            const entryCount = dataV.getUint16(
              byte + 9 + offsetToFirstIFD,
              isIntelAlign
            );
            ...
            for (let i = 0; i < entryCount; i++) {
              let tagNum = dataV.getUint16(
                byte + 9 + offsetToFirstIFD + 2 + i * 12,
                isIntelAlign
              );

              // Found Image Width (however for some files this is thumbnail width)
              if (tagNum === EXIF_IMAGE_WIDTH) {
                let actualWidth = dataV.getUint16(
                  byte + 9 + offsetToFirstIFD + 2 + i * 12 + 8,
                  isIntelAlign
                );
              ...
              if (tagNum === EXIF_IMAGE_HEIGHT) {
                let actualHeight = dataV.getUint16(
                  byte + 9 + offsetToFirstIFD + 2 + i * 12 + 8,
                  isIntelAlign
                );
